...
if ! grep -Eq "Current release: \\*\\*v5\\.4\\.0\\*\\*" README.md; then
  echo "README.md must state the current release as v5.4.0." >&2
  exit 1
fi

if ! grep -Eq "Current release: \\*\\*v5\\.4\\.0\\*\\*" ROADMAP.md; then
  echo "ROADMAP.md must state the current release as v5.4.0." >&2
  exit 1
fi
